...
### 2026-08-07 — 高頻移動監聽與車站檢索效能優化 (v1.8.0)

**修改原因：**
- 減少主執行緒在高頻移動監聽下的負載，避免 `PlayerMoveEvent` 頻繁調用 `RailUtil.findRailBlock` 進行實體世界方塊讀取 (I/O)，改善伺服器效能。

**修復/更新摘要：**
1. **`StopManager.java` — 空間查詢優化**：
   - 重構 `lookupGoldNear` 方法，移除對 `RailUtil.findRailBlock` 的呼叫與世界方塊的實體查詢。
   - 改為純記憶體空間位移檢測：直接從玩家所處的 Y 座標，向下/上依次檢測 y 偏移 0 到 -6 的座標，利用空間位移在已快取的 `goldBlockIndex` 內以 O(1) 速度檢索。
   - 新增 `hasStopsInWorld` 早期過濾方法，直接查詢某個世界是否註冊有任何車站金磚。
2. **`StationDisplayListener.java` — 移動監聽早期過濾**：
   - 在 `onMove` 方法中加入早期過濾條件：若當前玩家所在的世界完全沒有註冊任何車站金塊，則立即結束監聽事件處理，大幅減少 `updateDisplay` 方法的調用。

### 2026-07-30 — 站點導覽、路線規劃、轉乘站自動下車與聊天室分享系統 (v1.7.0)

**修改原因：**
- Avery 要求新增玩家專屬的 `/tr` 指令選單功能（純 GUI 面板化，無須指令），供玩家查詢最近站點、伺服器路線總覽、轉乘/重疊站一覽與乘車指引。
- 權限與全體玩家預設：
  1. `plugin.yml` 註冊 `atrain.user: default: true`。
  2. 非管理員玩家打 `/tr` **無須任何權限設定，預設直接對所有玩家開放**開啟「站點導覽」選單（除非被 LP 顯式將 `atrain.user` 設為 `false` 禁用）。
  3. 管理員打 `/tr` 開啟原管理員選單，面板中亦提供按鈕可隨時切換至導覽選單。
...
4. **權限與指令 (`TrainCommand` & `plugin.yml`)**：
   ...
   - `TrainCommand.openGui`: 預設開放所有非管理員玩家打 `/tr` 均可開啟導覽面板。
5. **版本號與構建**：`build.gradle.kts` 版本號更正為 `1.7.0`，Gradle 編譯通過 (BUILD SUCCESSFUL)。

### 2026-07-30 — 多人站點即時上車與順序 1 秒間隔發車防重疊機制

**修改原因：**
- Avery 回報舊有的站點召車機制會在站內有車時將玩家放入地面佇列等待，無法讓多人同時上車與在同一條軌道上運行。若讓多人同時發車又會導致車輛緊黏重疊視線混亂。
- 暫時保持版本號不變。

**修復/更新摘要：**
1. **`CartSpawnManager` 即時上車與防重疊**：移除地面排隊等待機制。玩家點擊站點鐵軌時，立即使用 `findAvailableRailBlock` 搜尋點擊處或站點內未被佔用的鐵軌格生成礦車並自動乘坐，徹底解決礦車重疊與無法即時上車的問題。
2. **`CinematicTransitManager` & `CinematicTransitTask` 順序發車**：新增 `stationLastDepartureTick` 排程維護。同一站點/路線/方向的多輛列車，發車時間自動排定為至少相隔 1 秒 (20 ticks)，讓後續上車的玩家依序發車，在軌道上保持距離不緊黏。
3. **`StationAutoStopListener` 相容更新**：在原版物理自動停靠模式下亦套用順序發車時間戳計算，確保多車發車均維持 1 秒以上間隔。
